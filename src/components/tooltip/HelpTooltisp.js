  ...
  return (
          ...
          {props.imageArray && props.imageArray.length > 0 && (
            <>
              {props.imageArray.map((value, index) => (
                <Grid key={index} container style={{ margin: "0  auto" }} spacing={1}>
                  <Grid item xs={8} sm={8}>
                    {value.imageTitle}
                  </Grid>
                  <Grid item xs={3} sm={3}>
    ...
    >
      {props.children ? (
        ...
        <FiHelpCircle className={props.helpIcon ? props.helpIcon : "gg-helpicon"} />
      )}
    </Tooltip>
  );
};
